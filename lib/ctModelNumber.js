...
export async function extractModelDataFromPage(page) {
  ...
  const raw = await page.evaluate(() => {
    ...
    const extractJsonLd = () => {
      ...
      for (const script of scripts) {
        ...
        for (const item of items) {
          ...
          const brand = dfsFindValue(item, ["brand"]);
          const upc = dfsFindValue(item, ["gtin12", "gtin13", "gtin", "upc"]);
          if (model || brand || upc) {
    ...
    const extractNextData = () => {
      ...
      const brand = dfsFindValue(parsed, ["brand"]);
      const upc = dfsFindValue(parsed, ["upc", "gtin", "gtin12", "gtin13"]);
      if (model || brand || upc) {
